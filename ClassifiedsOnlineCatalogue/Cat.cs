...
internal class Cat {
    ...
    private void viewHeader() {
        ...
        I386.POS_WriteNewLine("\t\t\t\t\t   Catálogo Online de Classificados");
    ...
    private void viewOrder(OrderList order) {
        ...
        for (int j = 0; j < order.parts.Count; ++j) {
            string translatedPartName = VinTranslations.GetTranslation(order.parts[j].partName);
            I386.POS_WriteNewLine($"\t\t\t\t\t++ {translatedPartName}");
        }
        I386.POS_WriteNewLine("\t\t\t\t\t-------------------------------------");
        ...
        if (!order.ordered) {
            ...
            if (error) {
                I386.POS_Write($" [ERRO] - {order.phoneNumber}");
            }
            else {
                I386.POS_Write($" [COMPRAR] - {order.phoneNumber}");
            }
        }
        else {
            I386.POS_Write(" [ENCOMENDADO]");
        }
        I386.POS_NewLine();
    }
    private void viewDownload() {
        ...
        switch (state) {
            ...
            case OrderState.DownloadError:
                I386.POS_Write($"\t\t\t\t\t             Erro no download");
                break;
        }
        ...
        if (state == OrderState.Downloaded) {
            I386.POS_WriteNewLine($"\t\t\t\t\t     Pressione Espaço para Continuar");
            if (I386.GetKeyDown(KeyCode.Space)) {
                state = OrderState.Viewing;
            }
        }
        ...
        if (state == OrderState.DownloadError) {
            I386.POS_WriteNewLine($"\t\t\t\t\t      Pressione Espaço para Tentar Novamente");
            if (I386.GetKeyDown(KeyCode.Space)) {
                state = OrderState.Connect;
            }
        }
    ...
    private void viewNewMagazine() {
        viewHeader();
        I386.POS_WriteNewLine($"\t\t\t\t\t Nova edição disponível para download");
        I386.POS_WriteNewLine($"\t\t\t\t\t     Pressione Espaço para Baixar");
        if (I386.GetKeyDown(KeyCode.Space)) {
